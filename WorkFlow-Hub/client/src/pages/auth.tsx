    ...
    if (isRegister) {
      ...
      try {
        const response = await fetch("https://lexington-data-entry-platform.onrender.com/api/register", {
          ...
          credentials: "include",
          body: JSON.stringify({ name, email: values.email, password: values.password, role: "worker" }),
        });
        ...
        if (response.ok) {
          ...
          toast({ title: "Error", description: errorData?.error || "An error occurred.", variant: "destructive" });
        }
      } catch (error) {
        toast({ title: "Error", description: "Network error or server is unreachable.", variant: "destructive" });
      }
    } else {
      try {
        ...
        toast({ title: "Login failed", description: "Network error or server is unreachable.", variant: "destructive" });
      }
    }
  };
          ...
          <TabsContent value="register">
            <Card className="border-none shadow-xl">
              <CardHeader>
                ...
                <CardDescription>Start earning money with simple data entry tasks.</CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...form}>
